...
def suru_iha_custom_triangle(target_latitude, target_longitude, target_altitude, drones):
    try:
        rospy.init_node('send_goto_position_node')
        
        # Define custom positions for each drone with increased spacing
        drone_positions = {
            '/drone3': (target_latitude, target_longitude + 0.001, target_altitude),  # Drone 1 at right of target
            '/drone2': (target_latitude - 0.0009, target_longitude, target_altitude),  # Drone 2 above target
            '/drone1': (target_latitude -0.0003, target_longitude - 0.001, target_altitude)   # Drone 3 at left of target
        }
...
if __name__ == '__main__':
    ...
    suru_iha_custom_triangle(target_latitude, target_longitude, target_altitude, drones)
